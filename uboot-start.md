...
# 物理扇区和逻辑扇区的区别，目前物理扇区大小为4096字节（即4KB）。为了保持与旧操作系统的兼容性，操作系统会模拟512字节的逻辑扇区。
...
首先，根据elf加载流程，动态链接的地址重定位是在可执行文件**执行过程中**在内核分配完内存空间和栈空间之后，调用ld.so动态连接器，转到用户空间加载依赖的共享库，然后进行运行时地址重定位。最终跳转程序入口，开始执行程序。
